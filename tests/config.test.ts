...
describe('loadConfig', () => {
  beforeAll(() => {
    testOverrides.readFileSync = jest.fn() as unknown as typeof testOverrides.readFileSync & jest.Mock;
    testOverrides.statSync = jest.fn() as unknown as typeof testOverrides.statSync & jest.Mock;
    testOverrides.env = {};
  });

  it('loads default config with no overrides', () => {
    (testOverrides.readFileSync as jest.Mock).mockImplementation(() => { throw new Error('File not found'); });
    (testOverrides.statSync as jest.Mock).mockImplementation(() => { throw new Error('File not found'); });
  ...
  it('applies valid .cursorrules overrides', () => {
    ...
    (testOverrides.readFileSync as jest.Mock).mockReturnValue(fileContent);
    (testOverrides.statSync as jest.Mock).mockReturnValue({ size: 100 });
  ...
  it('ignores oversized .cursorrules file', () => {
    (testOverrides.statSync as jest.Mock).mockReturnValue({ size: 20000 });
  ...
  it('ignores .cursorrules with invalid characters', () => {
    (testOverrides.readFileSync as jest.Mock).mockReturnValue('trustScoreThreshold=5.0<script>');
    (testOverrides.statSync as jest.Mock).mockReturnValue({ size: 100 });
  ...
  it('parses aiProviderConfig JSON in .cursorrules', () => {
    ...
    (testOverrides.readFileSync as jest.Mock).mockReturnValue(content);
    (testOverrides.statSync as jest.Mock).mockReturnValue({ size: 100 });
